...
import { TOOL_LABELS, LEAD_LABELS, RATING_LABELS } from "@/lib/activityPlanWord";
...
const ActivityIdeasWall = () => {
  ...
  const fetchIdeas = useCallback(async () => {
    const { data } = await supabase
      ...
      .select("id, created_at, activity_text, primary_tool, lead_stages, inclusion_rating, staff_name, show_name, department")
  ...
  const filtered = ideas.filter(i =>
    (tool === "all" || i.primary_tool === tool) &&
    (lead === "all" || i.lead_stages.includes(lead)) &&
            ...
            {filtered.map((idea) => (
              <div key={idea.id} className="bg-card rounded-2xl border border-border shadow-[var(--shadow-card)] p-5 hover:shadow-md transition-shadow">
                <div className="flex items-center gap-2 mb-3 flex-wrap">
                  ...
                  {idea.lead_stages.map(s => (
                    <span key={s} className={`text-[10px] px-2 py-0.5 rounded-full font-bold ${LEAD_BADGE[s]}`}>{LEAD_LABELS[s]}</span>
                  ))}
